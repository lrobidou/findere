...
int main(int argc, char* argv[]) {
    ...
    for (unsigned long long k_iter = k; k_iter > 13; k_iter -= 2) {
        ...
        for (double epsilonPercent_iter = 0.5; epsilonPercent_iter <= epsilonPercent; epsilonPercent_iter += 0.5) {
            ...
            if (bits) {
                std::tie(normalfilter, numberOfIndexedElements) = findere_internal::indexFastqGZGivenBits(input_filenames, bits, numHashes, k_iter, canonical);
            } else {
                std::tie(normalfilter, numberOfIndexedElements) = findere_internal::indexFastqGZGivenTruth(input_filenames, truthBigK, numHashes, k_iter, epsilonPercent_iter, canonical);
            }
            ...
            for (unsigned long long z_iter = 0; z_iter < z; z_iter++) {
                ...
                findere_internal::printContext(k_iter, z_iter, epsilonPercent_iter);

                if (bits) {
                    truthSmallK = truth::indexFastqGz(input_filenames, k_iter - z_iter, canonical);
                    std::tie(smallFilter, timeTakenMs, sizeOfBloomFilter) = findere::indexFastqGzGivenBits(input_filenames, numHashes, k_iter, z_iter, bits, canonical);
                } else {
                    std::tie(truthSmallK, smallFilter, timeTakenMs, sizeOfBloomFilter) = findere::indexFastqGz(input_filenames, numHashes, k_iter, epsilonPercent_iter, z_iter, canonical);
                }
                ...
                findere_internal::printScore(findere_internal::getScore(bigTruth, QTFOnSmallTruth), "resultsOnSmallTruth", false);
